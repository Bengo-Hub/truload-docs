...
| Offline capture / sync (TruConnect, frontend-less) | `TruConnect/src/backend/{SyncQueue,ConfigSyncService,BackendClient,ComplianceEngine,LocalWeighingStore}.js` | [Architecture](../technical/architecture.md#components) · [Offline mode implementation](../technical/offline-mode-implementation-plan.md#this-vs-truconnects-sqlite-offline-system) · [Auto-weigh flow](https://github.com/Bengo-Hub/TruConnect/blob/master/docs/AUTO_WEIGH_FLOW.md#local-provisional-compliance-offline-weighing-redesign-2026-09) | `TruConnect/tests/sync-queue-and-config-sync-test.js` (19/19), `TruConnect/tests/compliance-engine-test.js` (pure-function parity against `truload-frontend`'s own real-weighing fixtures), `TruConnect/tests/local-weighing-and-db-integration-test.js` (real SQLite, via `ELECTRON_RUN_AS_NODE=1` to match the compiled native module's ABI) — all real, all passing; this run caught and fixed a genuine bug (`ConfigSyncService`'s new upsert methods never actually executed, see the auto-weigh flow doc). **Still not verified: `TruConnect/scripts/live-demo-sync-check.js` against the real live demo backend (ready to run, blocked only by the implementing session's own sandboxed network egress), and an actual click-through of the Mobile/Multideck capture UI in a running Electron window (no GUI available in that sandbox).** |
