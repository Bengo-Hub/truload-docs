...
| Offline capture / sync (TruConnect, frontend-less) | `TruConnect/src/backend/{SyncQueue,ConfigSyncService,BackendClient,ComplianceEngine,LocalWeighingStore}.js` | [Architecture](../technical/architecture.md#components) · [Offline mode implementation](../technical/offline-mode-implementation-plan.md#this-vs-truconnects-sqlite-offline-system) · [Auto-weigh flow](https://github.com/Bengo-Hub/TruConnect/blob/master/docs/AUTO_WEIGH_FLOW.md#local-provisional-compliance-offline-weighing-redesign-2026-09) | `TruConnect/tests/sync-queue-and-config-sync-test.js` (19/19 real-transport behaviours, section (e)); `TruConnect/tests/compliance-engine-test.js` (pure-function parity against `truload-frontend`'s own real-weighing fixtures, no native-module dependency). **The SQLite-touching paths (migrations 017-019, the ConfigSyncService/BackendClient/LocalWeighingStore changes, and the Mobile/Multideck capture UI itself) are syntax-checked only, not yet integration- or click-through-tested; the implementing session's sandbox had no working native-module build toolchain and no Electron GUI.** |
